...
export const grammar = class RegexGrammar {
  ...
  *WordBoundaryAssertion({ ctx }) {
    ...
    yield defineAttribute('negate', ctx.sourceTextFor(m_.node) === 'B');
  }
  ...
  *DigitCharacterSet({ ctx }) {
    ...
    yield defineAttribute('negate', ctx.sourceTextFor(code.node) === 'D');
  }
  ...
  *SpaceCharacterSet({ ctx }) {
    ...
    yield defineAttribute('negate', ctx.sourceTextFor(code.node) === 'S');
  }
  ...
  *WordCharacterSet({ ctx }) {
    ...
    yield defineAttribute('negate', ctx.sourceTextFor(code.node) === 'W');
  }
  ...
  *Quantifier({ ctx }) {
    ...
    if ((sigil = yield eatMatch(m`sigilToken: <*Keyword /[*+?]/ />`))) {
      switch (ctx.sourceTextFor(sigil.node)) {
        case '*':
      ...
      min = min && ctx.sourceTextFor(min.node);
      max = max && ctx.sourceTextFor(max.node);
