...
import { printSource, printString, printTag } from '@bablr/agast-helpers/tree';
...
export default class Regex {
  ...
  constructor() {
    ...
    this.attributes = BMap.from(
      ...
      entry('WordBoundaryAssertion', freezeRecord({ negate: undefined })),
      entry('CharacterClass', freezeRecord({ negate: undefined })),
      entry('DigitCharacterSet', freezeRecord({ negate: undefined })),
      entry('SpaceCharacterSet', freezeRecord({ negate: undefined })),
      entry('WordCharacterSet', freezeRecord({ negate: undefined })),
      entry('Quantifier', freezeRecord({ min: undefined, max: undefined })),
    );
  }
  ...
  *Flags() {
    ...
    for (const flagChr of flagsStr) {
      yield eat(m`tokens[]*: <*Keyword ${printString(flagChr)} />`);
    }
  }
  ...
  *Elements({ matcher }) {
    while (yield match(m`/[^|]/`)) {
      yield eat(m`${printTag(matcher.value.reference)} <_Element />`);
    }
  }
  ...
  *Element({ s }) {
    ...
    } else if (yield eatMatch(m`<_CharacterSet /\.|\\[dsw]/i />`)) {
    } else {
      if (!(yield match(m`'\\'`))) {
  ...
  *Character() {
    if (yield match(m`'\\'`)) {
      yield eat(m`<EscapeSequence />`, o({}), o({ escape: true }));
    } else {
      yield eat(m`/[^\r\n\t]/`);
    }
  }
  ...
  *CharacterClassElement() {
    if (yield eatMatch(m`<_CharacterSet /\.|\\[dsw]/i />`)) {
    } else if (yield eatMatch(m`<Gap '\\g' />`)) {
    } else {
      yield eat(m`<*Character />`);
    }
  ...
  *EscapeSequence({ ctx, s }) {
    ...
    if ((m_ = yield match(m`/[\\/nrt0]/`))) {
      ...
      yield eat(m`code*: <*Keyword ${printString(cooked)} />`);
