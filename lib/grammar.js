...
export const grammar = class RegexGrammar {
  ...
  *CharacterClassElement({ value: props, ctx }) {
  ...
  @CoveredBy('CharacterClassElement')
  @Node
  *CharacterClassRange({ value: props, ctx }) {
    const { first = i.Expression`false` } = ctx.unbox(props || {}) || {};
  ...
  *Quantifier() {
    ...
    yield i`bindAttribute('min' ${buildNumber(attrs.min)})`;
    yield i`bindAttribute('max' ${buildNumber(attrs.max)})`;
  }
