export class RegexMiniprinterGrammar {
  RegExpLiteral(p) {
    p.print('/');
    p.pushSpan({ type: 'Expression' });
    p.eatProduction('Pattern', { path: 'pattern' });
    p.popSpan();
    p.print('/');
  ...
  CharacterClassRange(p) {
    p.eatProduction('Character', { type: 'min' });
    p.replaceSpan({ type: 'CharacterClass' });
    p.print('-');
    p.eatProduction('Character', { type: 'max' });
  ...
  Quantifier(p, { node }) {
    ...
    p.eatProduction('Element', { type: 'element' });
