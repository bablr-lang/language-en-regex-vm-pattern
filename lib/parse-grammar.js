...
const specialPatterns = {
  ...
  CharacterClass: t.Regex(t.Pattern([t.Alternative([t.CharacterClass(t.Characters('].\\\n'))])]), [
    t.Flags('y'),
  ]),
  'CharacterClass:NoRange': t.Regex(
    t.Pattern([t.Alternative([t.CharacterClass(t.Characters(']-.\\\n'))])]),
    t.Flags('y'),
  ),
  'CharacterClass:NoNegate': t.Regex(
    t.Pattern([t.Alternative([t.CharacterClass(t.Characters(']^.\\\n'))])]),
    t.Flags('y'),
  ),
};

export class RegexGrammar {
  constructor() {
  ...
  *Character() {
    ...
    const esc = yield eatMatch(t.TokenMatcher('Token', t.StringMatcher('\\')));
    let value;
    ...
    if (esc) {
      ...
      if (escapeType) {
        ...
        value = yield eat(specialPattern);
      }
    } else if (!(yield match(specialPattern))) {
      value = yield eat(t.RegexMatcher(t.Regex(t.Pattern([t.CharacterSet('any')]), t.Flags('y'))));
    } else {
